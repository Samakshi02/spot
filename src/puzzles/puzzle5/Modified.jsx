...
export function Puzzle5Modified({ onFound, foundIds, hintingId }) {
  return (
    <div className="w-full h-full rounded-2xl border border-slate-200 bg-white shadow-sm overflow-visible flex flex-col max-w-sm mx-auto p-5">
      <div className="w-36 h-36 rounded-lg mx-auto mb-4 overflow-hidden relative shadow-lg" style={{ background: 'linear-gradient(160deg, #0f0c29 0%, #302b63 40%, #24243e 100%)' }}>
        ...
        <div className="absolute bottom-2 left-0 right-0 text-center">
          <div className="text-[7px] font-bold uppercase tracking-[0.25em] text-white/90">Midnight Ride</div>
      ...
      <DiffZone id={3} index={3} onFound={onFound} found={foundIds.includes(3)} hinting={hintingId === 3} className="w-full">
        <h3 className="text-base font-semibold text-slate-900 text-center">Midnight Ride</h3>
      </DiffZone>
      ...
      <div className="flex items-center justify-center gap-4 mb-4">
        <DiffZone id={2} index={2} onFound={onFound} found={foundIds.includes(2)} hinting={hintingId === 2}>
          <button type="button" className="w-10 h-10 flex items-center justify-center rounded-full border border-slate-300 text-slate-400" aria-label="Shuffle">
        ...
        <DiffZone id={1} index={1} onFound={onFound} found={foundIds.includes(1)} hinting={hintingId === 1}>
          <button type="button" className="w-12 h-12 flex items-center justify-center rounded-full bg-slate-900 text-white" aria-label="Pause">
      ...
      <div className="flex items-center gap-2">
        <DiffZone id={5} index={5} onFound={onFound} found={foundIds.includes(5)} hinting={hintingId === 5}>
          <button type="button" className="w-8 h-8 flex items-center justify-center text-slate-400" aria-label="Muted">
            <span className="text-lg">🔇</span>
          </button>
        </DiffZone>
